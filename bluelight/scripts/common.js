...
function displayMark(viewportNum0, o3DElement) {
    ...
    if (NowResize == false) {//if (size == false) {
        sizeCheck = true;
        if (!o3DElement) {
    ...
    tempctx.lineWidth = "" + getMarkSize(MarkCanvas, false); //((Math.abs(lineWid)) * 2 * lineSize);
    NowResize = false;
            ...
            if (PatientMark[n].sop == Patient.Study[i].Series[j].Sop[k].SopUID) {
                for (var m = 0; m < PatientMark[n].mark.length; m++) {
                    if (PatientMark[n].mark[m].type == "XML_mark") {
                        var tempAlpha = tempctx.globalAlpha;
                    ...
                    if (PatientMark[n].mark[m].type == "POLYLINE") {
                        for (var o = 0; o < PatientMark[n].mark[m].markX.length; o += 1) {
                            ...
                            var tempAlpha2 = tempctx.globalAlpha;
                            tempctx.globalAlpha = 1.0;
...
function SetWindowWL(openOrigin) {
    ...
    if (openLink == true) {
        ...
        for (var z = 0; z < Viewport_Total; z++) {
            ...
            if (uid) {
                if (z == viewportNumber)
                    loadAndViewImageByWindowLevwl(Patient.Study[uid.studyuid].Series[uid.seriesuid].Sop[uid.sopuid].imageId, parseInt(textWC.value), parseInt(textWW.value), openOrigin, z);
                else
                    loadAndViewImageByWindowLevwl(Patient.Study[uid.studyuid].Series[uid.seriesuid].Sop[uid.sopuid].imageId, parseInt(textWC.value), parseInt(textWW.value), false, z);
            }
        }
    } else {
        ...
        var uid = SearchUid2Json(alt);
        loadAndViewImageByWindowLevwl(Patient.Study[uid.studyuid].Series[uid.seriesuid].Sop[uid.sopuid].imageId, parseInt(textWC.value), parseInt(textWW.value), openOrigin);
    }
}
