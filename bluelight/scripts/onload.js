  ...
  return new Promise((resolve, reject) => {
    ...
    request.onload = function() {
      if (request.status === 200) {
        ...
        console.error("HTTP error for instance request:", request.status);
        showDicomStatus("PACS error: Failed to retrieve instance (Status " + request.status + ")", true);
        resolve([]); // 回傳空陣列代表無資料
      }
    };
...
async function getAllInstances(baseUrl) {
  ...
  while(true) {
    ...
    try {
      ...
      console.error("Network error occurred while fetching instance data", error);
