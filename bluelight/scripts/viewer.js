...
/**  
 * Handles 204 (No Content) responses for StudyInstanceUID queries  
 * @param {string} url - The URL that was queried  
 * @param {Element} container - Optional container to display the error message  
 */
function handleStudyNotFound(url, container) {
  ...
  // If we have the showErrorMessage function from our previous implementation, use it  
  if (typeof showErrorMessage === 'function') {
    showErrorMessage(`The requested StudyInstanceUID (${studyUid}) does not exist in the PACS server.`);
    return;
  }

  // Otherwise, create a simple error message  
  const errorDiv = document.createElement("div");
  errorDiv.className = "alert alert-danger";
  errorDiv.style.margin = "20px";
  errorDiv.innerHTML = `<strong>Error:</strong> The requested StudyInstanceUID (${studyUid}) does not exist in the PACS server.`;

  // Insert the error message  
  if (container) {
    container.insertBefore(errorDiv, container.firstChild);
  } else {
    const defaultContainer = document.querySelector(".center") || document.body;
    defaultContainer.insertBefore(errorDiv, defaultContainer.firstChild);
  }
...
function limitedFetch(fetchFunc) {
    if (currentFetchCount < MAX_CONCURRENT_FETCH) {
        ...
        .catch(error => {  
            ...
            if (typeof showErrorMessage === 'function') {  
              showErrorMessage(error.message || `Error connecting to PACS server: ${error}`);  
            } else {  
              alert(error.message || `Error connecting to PACS server: ${error}`);  
            }  
        }) 
        .finally(() => {
            currentFetchCount--;
        ...
        fetch(url, { headers, }).then(async function (res) {
            if (!res.ok) {  
                ...
                switch (res.status) {  
                    ...
                    case 401:  
                    case 403:  
                        errorMessage = `Authentication error (${res.status}): You don't have permission to access this DICOM resource.`;  
                        break;  
                    default:  
                        errorMessage = `HTTP error ${res.status} (${res.statusText}) occurred while connecting to PACS server.`;  
                }  
